...
export function validateQuotePayload(
  ...
  if (shouldValidate("productId") && !cleanString(input.productId)) errors.push({ field: "productId", message: "Product is required" });
  if (shouldValidate("quantity") && !isPositiveNumber(input.quantity)) errors.push({ field: "quantity", message: "Quantity must be greater than 0" });
  if (shouldValidate("unitPrice") && !isPositiveNumber(input.unitPrice)) errors.push({ field: "unitPrice", message: "Unit price must be greater than 0" });
  if (shouldValidate("currency") && !cleanString(input.currency)) errors.push({ field: "currency", message: "Currency is required" });
  if (input.shippingCost !== undefined && input.shippingCost !== null && input.shippingCost !== "" && Number(input.shippingCost) < 0) {
  ...
  if (input.tiers?.some((tier) => !isPositiveNumber(tier.quantity) || !isPositiveNumber(tier.unitPrice))) {
    errors.push({ field: "tiers", message: "Tier quantity and unit price must be greater than 0" });
  }
  return errors;
}

export function buildQuoteResponse(input: QuoteInput, product: ProductDetail, knowledgeItems: KnowledgeContextItem[] = []): QuoteResponse {
  const currency = cleanString(input.currency) || "USD";
  const quantity = Number(input.quantity);
  const unitPrice = Number(input.unitPrice);
  const shippingCost = input.shippingCost === undefined || input.shippingCost === null || input.shippingCost === "" ? null : Number(input.shippingCost);
  ...
  const quoteText =
    "quoteText" in input && input.quoteText?.trim()
      ...
      : withKnowledgeLine(
          buildQuoteText({ product, language, quantity, unitPrice, currency, shippingCost, moq, leadTime, includeShipping, tiers }),
          knowledgeUsed
        );
  const riskWarnings = buildRiskWarnings(input, product, unitPrice, shippingCost, quoteText, knowledgeItems);
...
function buildEnglishQuote(input: Parameters<typeof buildQuoteText>[0]) {
  const tierText =
    input.tiers.length > 0
      ? `\nTier price:\n${input.tiers.map((tier) => `- ${tier.quantity} pcs: ${input.currency} ${money(tier.unitPrice)} / pc`).join("\n")}`
      : "";
  const shipping =
    input.shippingCost === null
      ? "Shipping cost: to be confirmed based on destination city and shipping method."
      : `Shipping cost: ${input.currency} ${money(input.shippingCost)}${input.includeShipping ? " (included in total offer)" : " (not included in unit price)"}.`;
  return [
...
function buildSpanishQuote(input: Parameters<typeof buildQuoteText>[0]) {
  return buildEnglishQuote(input)
    ...
    .replace("Please confirm your destination country, city, and preferred shipping method before final confirmation.", "Por favor confirma el pais, la ciudad de destino y el metodo de envio antes de la confirmacion final.")
    .replace("This is a draft quotation. We will confirm price, stock, lead time, and shipping before sending the final offer.", "Esta es una cotizacion en borrador. Confirmaremos precio, stock, plazo de entrega y envio antes de enviar la oferta final.");
}
...
function buildPortugueseQuote(input: Parameters<typeof buildQuoteText>[0]) {
  return buildEnglishQuote(input)
    ...
    .replace("Please confirm your destination country, city, and preferred shipping method before final confirmation.", "Por favor confirme o pais, a cidade de destino e o metodo de envio antes da confirmacao final.")
    .replace("This is a draft quotation. We will confirm price, stock, lead time, and shipping before sending the final offer.", "Esta e uma cotacao em rascunho. Vamos confirmar preco, estoque, prazo e frete antes de enviar a oferta final.");
}

function buildRiskWarnings(input: QuoteInput, product: ProductDetail, unitPrice: number, shippingCost: number | null, quoteText: string, knowledgeItems: KnowledgeContextItem[]) {
  ...
  if (quoteText.toLowerCase().includes("attached") && !input.attachmentSelected) warnings.push("文案中出现 attached，但未选择附件，请删除该表述或先确认附件。");
  return Array.from(new Set(warnings));
}

function withKnowledgeLine(quoteText: string, knowledgeUsed: string[]) {
  if (knowledgeUsed.length === 0) return quoteText;
  return `${quoteText}\nReference checked: ${knowledgeUsed.join(", ")}. Please confirm price, stock, lead time, shipping, payment terms, and company policy before sending.`;
}
